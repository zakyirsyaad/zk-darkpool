...
export function useTokenBalance(symbol: string) {
  ...
  const tokenAddress = chainId
    ? TOKEN_ADDRESSES[upperSymbol]?.[chainId]
    : undefined;

  // For native ETH: only when tidak ada contract address di chain tersebut
  const isNative = upperSymbol === "ETH" && !tokenAddress;
  ...
  const shouldFetchToken =
    !isNative && !!tokenAddress && !!address && isConnected;
  const {
    data: tokenData,
